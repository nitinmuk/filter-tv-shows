    ...
    describe("router", () => {
      describe("/show/drm-enabled-shows => post", () => {
        it("must be a router that accepts POST verb", async () => {
          ...
          const stubbedFilterDRMEnabledShows = sinon
            .stub(showController, "filterDRMEnabledShows")
            .resolves({ status: 200, json: response });
          const stubbedJoiMiddleware = sinon.stub(
          ...
          sinon.assert.calledOnceWithExactly(
            stubbedJoiMiddleware,
            drmEnabledShowSchema
          );
